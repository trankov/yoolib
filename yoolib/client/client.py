...
class YooEndpoints:
    """
    Generate endpoint strings for YooClient methods
    """
    ...
    def info(self) -> str:
        return self._build_url(self.points.INFO)
    ...
    def _build_url(self, point: YooPoints, **kwargs) -> str:
        return f'{self.points.BASE}{point.url(**kwargs)}'
...
class YooClientConnection:
    ...
    def python_version_signature(self):
        return '{}.{}.{} ({})'.format(*sys.implementation.version)  # pylint: disable=C0209

    @property
    def headers_ua(self):
        return (
            f'{platform.uname().system}/{platform.uname().release} '
            f'({platform.uname().machine}) '
            f'Python/{self.python_version_signature} '
            f'YooLib/0.2.0 (development)'
        )
...
class YooClient:
    ...
    def __init__(
        ...
        endpoints: YooEndpoints | None = None,
        connection: YooClientConnection | None = None,
        api_key: str | None = None,
        shop_id: str | None = None,
    ) -> None:
        if not hasattr(self, 'idempotence_key'):
            self.idempotence_key = idempotence_key or uuid4()
        if not hasattr(self, 'endpoints'):
            self.endpoints = endpoints or YooEndpoints()
        if not hasattr(self, 'connection'):
            if None in (api_key, shop_id):
                raise YooError('You must specify both an api_key and a shop_id')
            self.connection = connection or YooClientConnection(
                headers={'Idempotence-Key': str(self.idempotence_key)},
                api_key=api_key,
                shop_id=shop_id,
            )
    ...
    def _process_response(self, response: requests.Response) -> dict:
        if response.status_code not in (200, 201, 204):
            raise YooError(f'{response.status_code}, {response.text}').as_status(
